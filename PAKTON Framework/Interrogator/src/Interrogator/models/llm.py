"""
Description:
LLM factory that provides configured language models for different nodes, supporting OpenAI, Bedrock, Google, and local model APIs with node-specific configurations.
...
def get_default_llm(node_name=None):
    ...
    Each model config should have keys:
        - "API": either "openai", "bedrock", "google", or "local"
        - "model_id": the model ID to use
        - "args": dictionary containing model parameters like max_tokens, temperature, etc.

    Returns:
        An instance of the configured LLM (ChatOpenAI, BedrockLLM, ChatGoogleGenerativeAI, or local LLM).
    """
    ...
    if not models_config:
        # For backward compatibility, check for flat models config
...
def _create_llm_from_config(models_config, config_name):
    ...
    if api == "bedrock":
        ...
        raise ValueError(f"Unsupported API '{api}'. Please choose either 'openai', 'bedrock', 'google', or 'local'.")