"""
Description:
Configuration management singleton that loads and provides access to YAML configuration settings with support for nested key access using dot notation.
...
class Config:
    ...
    def _load_config(self):
        ...
        config_paths = [
            Path.cwd() / "config.interrogator.yaml",           # CWD alternative naming
            Path(__file__).parent.parent / "config.yaml",      # Interrogator/src/Interrogator/config.yaml
            Path("/app/Interrogator/src/Interrogator/config.yaml"),  # Docker mount location
            Path("/app/API/config.interrogator.yaml"),         # API directory with specific name
        ]
        
        # Try pkg_resources first (for installed packages)
        try:
            pkg_config_path = pkg_resources.resource_filename("Interrogator", "config.yaml")
